...
public class HoaDonVip extends HoaDon {
    public HoaDonVip() {
        super();
        this.loaiHoaDon = "HoaDonVIP";
    }

    @Override
    public void input() {
        System.out.println("Nhap vao hoa don khach hang VIP !");
    ...
    @Override
    public void output() {
        System.out.println("\nThong tin hoa don khach hang VIP !");
    ...
    public double tienKM() {
        ...
        if(super.getSoLuong() > 60) {
            for(String item : super.getListMathang()) {
                if(item.toLowerCase().contains(super.getIdMatHangMua().toLowerCase())) {
                    switch (super.getIdMatHangMua().toLowerCase()) {
                        case "ron92" -> tienGiam = (22000 * 0.05) * super.getSoLuong();
                        case "ron95" -> tienGiam = (25000 * 0.05) * super.getSoLuong();
                        case "e20" -> tienGiam += (15000 * 0.05) * super.getSoLuong();
                        default -> tienGiam += 0;
                    }
                }
            }
        } else if(super.getSoLuong() < 10) {
            tienGiam = this.thanhTien() * 0.01;
        } else {
            if(this.thanhTien() > 600000) {
                tienGiam = this.thanhTien() * 0.04;
            }
        }
        return tienGiam;
    }
    ...
    public double thanhToanTamUng() {
        return tongThanhTien() * 0.04;
    }

    public double laiSuatTraCham() {
        return (tongThanhTien() - thanhToanTamUng()) * 0.02;
    }
}
